...
export default class extends Service<Env> {
  ...
  private async callAI(config: {
    ...
    try {
      ...
      // OPTIMIZATION: Retry rate limits with exponential backoff before falling back
      if (isRateLimit && retryCount < 3) {
        const delay = Math.pow(2, retryCount) * 1000; // 1s, 2s, 4s
        this.env.logger.info(`🔄 Rate limit detected, retrying in ${delay}ms (attempt ${retryCount + 1}/3)`, {
          model: modelName,
          retryDelay: delay
        });

        await new Promise(resolve => setTimeout(resolve, delay));
        return this.callAI({ ...config, retryCount: retryCount + 1 });
      }
      ...
      this.env.logger.warn('⚠️ Cerebras API failed, attempting fallback to Raindrop AI', {
        ...
        duration,
        isRateLimit,
        retriesExhausted: isRateLimit && retryCount >= 3,
        isTimeout: errorMsg.includes('timeout')
      });

      // FALLBACK: Use Raindrop AI (Workers AI) when Cerebras is unavailable or retries exhausted
      try {
  ...
  async chat(workspaceId: string, userId: string, request: ChatRequest): Promise<ChatResponse> {
    this.env.logger.info('🚨 CHAT METHOD CALLED', {
      workspaceId,
      userId,
    ...
    const baseSystemPrompt = `You are AuditGuardX AI, your friendly compliance expert and assistant.
...
- DO NOT answer compliance questions from memory - always check the knowledge base first`;
    ...
    try {
      ...
      const memoryEntries = await workingMemory.getMemory({
        ...
        nMostRecent: 100, // Last 50 turns (100 messages: 50 user + 50 assistant)
      });
    ...
    // OPTIMIZATION: Truncate to last 10 messages to prevent token explosion and Cerebras rate limits
    const truncatedHistory = processedHistory.slice(-10);
...
⚠️ CONVERSATION CONTINUATION (CRITICAL - PREVENTS LOOPS):
- Check the PREVIOUS assistant message in conversation history
- If it contained a question/offer AND user responds with affirmative → EXECUTE the implied action NOW
...
🚨 REMEMBER: When in doubt, USE TOOLS! Never guess or answer from memory about workspace data!`
      ...
      if (!messages || !Array.isArray(messages) || messages.length === 0) {
        throw createUserFriendlyError(
          'Cannot process empty conversation',
          'INVALID_REQUEST'
        );
      }
      ...
      try {
        // OPTIMIZATION: Send only last 3 messages to decision stage to reduce tokens by 70%
        // Decision maker only needs immediate context, not full conversation history
        const decisionMessages = messages.slice(-3);

        decisionResponse = await this.callAI({
          model: 'decision',
          messages: [
            decisionPrompt,
            ...decisionMessages // Last 3 messages only (optimized for Cerebras rate limits)
          ] as any,
          responseFormat: { type: 'json_object' },
          temperature: 0.3,
          maxTokens: 500,
      ...
      // Log raw AI response BEFORE parsing (with safe substring)
      ...
      // OPTIMIZATION: Add delay between decision and response stages to prevent Cerebras queue collision
      // This prevents both API calls from hitting the queue simultaneously
